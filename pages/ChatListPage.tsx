...
const ChatListPage: React.FC = () => {
  ...
  const handleBlock = (userName: string) => {
    if (window.confirm(`Are you sure you want to block ${userName}? You will no longer see their profile or receive messages from them.`)) {
...
const ChatListItem: React.FC<ChatListItemProps> = ({ chat, onDelete, onBlock }) => {
    ...
    return (
        <div className="relative">
            ...
            <button onClick={() => setMenuOpen(!menuOpen)} className="absolute top-1/2 right-4 -translate-y-1/2 text-gray-400 hover:text-gray-600">
                <EllipsisVerticalIcon className="h-6 w-6" />
            </button>
            {menuOpen && (
                <div className="absolute top-12 right-6 bg-white dark:bg-gray-700 rounded-md shadow-lg z-10 w-40 text-sm">
                    <button onClick={() => { onBlock(chat.userName); setMenuOpen(false); }} className="block w-full text-left px-4 py-2 text-red-600 dark:text-red-400 hover:bg-gray-100 dark:hover:bg-gray-600">Block User</button>
                    <button onClick={() => { onDelete(chat.id); }} className="block w-full text-left px-4 py-2 text-red-600 dark:text-red-400 hover:bg-gray-100 dark:hover:bg-gray-600">Delete Chat</button>
                </div>
            )}
        </div>
    );
