...
const MyProfilePage: React.FC = () => {
  ...
  const menuItems = [
    { label: 'کسانی که پروفایل من را دیدند (Visitors)', to: '/visitors', icon: UserCircleIcon },
    { label: 'کسانی که من را لایک کردند (Likes)', to: '/likes', icon: HeartIcon },
    { label: 'نشان شده ها (Bookmarks)', to: '/bookmarks', icon: BookmarkIcon },
    { label: 'بلاک شده ها (Blocked)', to: '/blocked', icon: UserCircleIcon },
  ];
      ...
      <div className="p-6 text-center">
        ...
        <div className="mt-4 flex justify-center space-x-4">
            <Link to="/edit-profile" className="flex-1 bg-gray-200 dark:bg-gray-700 text-gray-800 dark:text-gray-200 font-semibold py-2 px-4 rounded-lg flex items-center justify-center">
                <Cog6ToothIcon className="h-5 w-5 mr-2" /> Edit Profile
            </Link>
        </div>
      </div>
      
      <div className="px-4">
        <Link to="/store" className="block w-full text-left p-4 mb-4 rounded-lg bg-gradient-to-r from-pink-500 to-orange-400 text-white shadow-lg">
          <div className="flex items-center justify-between">
            <div>
              <h3 className="font-bold text-lg">ارتقا پنل به اشتراک ویژه</h3>
              <p className="text-sm">Get more features with Premium!</p>
            </div>
            ...
            return (
                <Link key={item.to} to={item.to} className="flex items-center justify-between p-4 bg-white dark:bg-gray-800 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
                    <div className="flex items-center">
                        <Icon className="h-6 w-6 text-pink-500 mr-4" />
