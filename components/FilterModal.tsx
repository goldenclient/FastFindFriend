  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl w-full max-w-md p-6 relative animate-fade-in-up">
        ...
        <h2 className="text-2xl font-bold mb-6 text-gray-900 dark:text-white">Filters</h2>
        ...
        <div className="mb-4">
            ...
            <div className="flex space-x-2">
                {(['All', ...Object.values(Gender)]).map(g => (
                    <button key={g} onClick={() => setGender(g as Gender | 'All')} className={`px-4 py-2 rounded-full text-sm font-semibold transition-colors ${gender === g ? 'bg-pink-500 text-white' : 'bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-200'}`}>
                        {g}
                    </button>
                ))}
            </div>
        ...
        {/* Age Range */}
        <div className="mb-4">
            <label htmlFor="age" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Age Range: {ageRange.min} - {ageRange.max}</label>
